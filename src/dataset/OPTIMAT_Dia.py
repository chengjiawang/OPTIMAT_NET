...
class OPTIMAT_Dia(torch.utils.data.Dataset):
    def __init__(self,
                 path,
                 opt,
                 train=True,
        ...
        super(OPTIMAT_Dia, self).__init__()
        self.path = path
        self.train = train
        self.make_one_hot = make_one_hot
        self.__original_image_size = None
        self.return_crop = return_crop
        self.fixSize = crop_size
        self.initialize(opt=opt)

    def extract_patient_name(self, in_string, final_word):
        return in_string[:-len(final_word)]

    def batch_crop(self, x, x_loc, fix_size=16, nd=2):
        xy, xx, xz = x_loc
        xy = torch.floor(xy).int()
        xx = torch.floor(xx).int()
        xz = torch.floor(xz).int()
        if nd == 2:
            xy_l, xy_u = xy - fix_size, xy + fix_size
            xx_l, xx_u = xx - fix_size, xx + fix_size
            # print(xy_u - xy_l)
            ...
            xy_l, xy_u = xy - fix_size, xy + fix_size
            xx_l, xx_u = xx - fix_size, xx + fix_size
            xz_l, xz_u = xz - fix_size, xz + fix_size
            return x[..., xy_l:xy_u, xx_l:xx_u, xz_l:xz_u]


    def initialize(self, opt):
        self.opt = opt
        if opt.dataset.lower() == 'totalhip' or opt.dataset.lower() == 'totalhipo':
            self.data_key_word = '*' + 'Neck' + '.nii'
            self.label_key_word = '*' + '_TotalHip.nii'
        else:
            self.data_key_word = '*' + opt.dataset + '.nii'
            self.label_key_word = '*' + opt.dataset + '_obj.nii'
        self.__image_size = None
        self.uniform_size = None

        self.data_files = _make_dataset3D(
            dir=self.path,
            key_word=self.data_key_word
        )
        self.data_files.sort()

        self.label_files = _make_dataset3D(
            dir=self.path,
            key_word=self.label_key_word
        )
        self.label_files.sort()

        self.train_arb_p = 1.0

        self.data_num = len(self.data_files)
        self.label_num = len(self.label_files)

        if self.data_num != self.label_num:
    ...
    def re_cache(self):
        ...
        # selecrted B
        self.selected_data = [
            self.data_files[i] for i in self.selected_patient
        ]

        self.selected_label = [
            self.label_files[i] for i in self.selected_patient
        ]

        self.patient_names = [self.extract_patient_name(
            self.data_files[i], self.data_key_word
        ...
        self.cached_volumes = []
        self.cached_labels = []
        self.cached_original_size = []
        for n_patient,patient in enumerate(self.patient_names):

            data = self.selected_data[n_patient]
            label = self.selected_label[n_patient]

            print('caching ', data, '...')
            print('caching', label, '...')

            volume_data, volume_header = nifti_reader_3D(
                data
            )

            self.cached_original_size += [np.array(volume_data.shape)]
            self.original_image_size = np.array(volume_data.shape)

            volume_label, label_header = nifti_reader_3D(
                label
            )


            # padding z
            # if volume_data.shape[-1] == 44:
            ...
            if hasattr(self.opt, 'resolution') and self.opt.resolution !=1:
                ...
                volume_label = ndi.zoom(volume_label,
                                             zoom=[scale_factorB,
                                                   scale_factorB,
                                                   scale_factorB],
                                              order=3, mode='constant')
            ...
            volume_label = _pad_to_shape_for_downsample3D(
                volume_label
            )

            volume_data = normalize_white(volume_data)

            self.cached_volumes += [volume_data]
            self.cached_labels += [volume_label]

            # get a proper subvolume size
            image_sizes += [
                np.array(volume_data.shape)
            ]
    ...
    def get_item_train(self, index):
        ...
        # extract coordinate
        # print('subvolume label size: ', sub_vol_label.shape)

        # if self.make_one_hot:
        #     center = torch.mean(
        #         torch.nonzero(sub_vol_label[-1] - sub_vol_label[-1].min()).float(),
        #         dim=0
        #     )
        # else:

        if torch.mean(sub_vol_label[-1]) == torch.min(sub_vol_label[-1]):
            center = torch.Tensor([
                sub_vol_label.shape[-3],
                sub_vol_label.shape[-2],
                sub_vol_label.shape[-1]
            ]).to(sub_vol_label.device) / 2.0
        ...
        # print('range_label: ', torch.min(sub_vol_label), torch.max(sub_vol_label))

        if not self.return_crop:
            return sub_vol_data, sub_vol_label, center
        else:
            # crop subvolume label
            crop_sub_vol_data = self.extract_subvolumn3D(sub_vol_data,
                                                         center,
                                                         [self.fixSize*2]*3)
            crop_sub_vol_label = self.extract_subvolumn3D(sub_vol_label,
                                                          center,
                                                          [self.fixSize*2]*3)
            print(crop_sub_vol_label.shape)
            return sub_vol_data, sub_vol_label, center, crop_sub_vol_data.float(), crop_sub_vol_label.float()
    ...
    def get_item_test(self, index):
        ...
        if self.original_image_size is None:
            self.original_image_size = np.array(volume_label.shape)
        ...
        temp = self.test_transform([volume_data, volume_label])
        volume_data = temp[0].unsqueeze(0)
        volume_data = self.normalize(volume_data)
        volume_label = temp[1].unsqueeze(0)



        if self.make_one_hot:
            volume_label = make_one_hot(
                volume_label,
                C=torch.max(volume_label)+1
            )
    ...
    def extract_subvolumn3D(self, vol, centre_coord, subvolumn_size, limit_within_vol=True):
        if not isinstance(subvolumn_size, Iterable):
            subvolumn_size = [subvolumn_size*2]*3
        vol_minx, vol_maxx, target_minx, target_maxx = \
            extract_sub_range(vol.shape[-3],
                              centre_coord[0],
                              subvolumn_size[0],
                              limit_within_vol=limit_within_vol)
        vol_miny, vol_maxy, target_miny, target_maxy = \
            extract_sub_range(vol.shape[-2],
                              ...
                              subvolumn_size[1],
                              limit_within_vol=limit_within_vol)
        vol_minz, vol_maxz, target_minz, target_maxy = \
            extract_sub_range(vol.shape[-1],
                              centre_coord[2],
                              subvolumn_size[2],
                              limit_within_vol=limit_within_vol)

        return vol[...,vol_minx:vol_maxx, vol_miny:vol_maxy, vol_minz:vol_maxz]

class OPTIMAT_Dia_loader(DataLoader):
    def __init__(self, dataset, batch_size=1, shuffle=False, sampler=None,
                 batch_sampler=None, num_workers=0, pin_memory=False, drop_last=False, timeout=0,
                 worker_init_fn=None
                 ):
        super(OPTIMAT_Dia_loader, self).__init__(
            dataset=dataset,
...
def get_OPTIMAT_loaders(
        opt,
        data_root = os.path.abspath('./OPTIMATData/OPTIMAT1'),
        batch_size = 2,
        num_workers = 6,
        return_crop = True
):
    ...
    trainset = OPTIMAT_Dia(
        path=train_path,
        opt=opt,
        train=True,
        return_crop=return_crop
    )

    train_loader = OPTIMAT_Dia_loader(dataset=trainset,
                              batch_size=batch_size,
                              shuffle=True,
                              num_workers=num_workers)
    ...
    val_loader = OPTIMAT_Dia_loader(dataset=valset,
                                batch_size=1,
                                shuffle=False,
                                num_workers=num_workers)
...
if __name__=='__main__':
    ...
    test_loader = OPTIMAT_Dia_loader(
        dataset=testset,
        batch_size=2,
        shuffle=False,
        num_workers=1
