...
def elastic_transform_2D(image,
    ...
    import numbers
    if alpha is None:
        alpha = np.array(list(image.shape)) * 2
    ...
    if sigma is None:
        sigma = np.array(list(image.shape)) * 0.08
    ...
    res_image = map_coordinates(image, indices, order=1, mode='reflect').reshape(shape)

    if mask is not None:
        res_mask = map_coordinates(mask, indices, order=1, mode='reflect').reshape(shape)
...
class RandomElasticDeform2D(transformation_base_class):
    def __init__(self,
                 freq = 0.9,
                 random_state = None):
        super(RandomElasticDeform2D, self).__init__()
        self.freq = freq
        if random_state is None:
    ...
    def __get_params__(self,
        ...
        import numbers
        if alpha is None:
            self.alpha = np.array(list(shape)) * 1
        elif isinstance(alpha, numbers.Number):
        ...
        if sigma is None:
            self.sigma = np.array(list(shape)) * 0.1
        elif isinstance(sigma, numbers.Number):
...
class RandomElasticDeform2D_test(transformation_base_class):
    def __init__(self,
                 freq = 0.9,
                 random_state = None):
        super(RandomElasticDeform2D_test, self).__init__()
        self.freq = freq
        if random_state is None:
            self.random_state = np.random.RandomState(None)
        else:
    ...
    def __get_params__(self,
        ...
        import numbers
        if alpha is None:
            self.alpha = np.array(list(shape)) * 2
        elif isinstance(alpha, numbers.Number):
            self.alpha = np.array([alpha, alpha])
        else:
            self.alpha = alpha
        ...
        if sigma is None:
            self.sigma = np.array(list(shape)) * 0.16
        elif isinstance(sigma, numbers.Number):
            self.sigma = np.array([sigma, sigma])
        else:
            self.sigma = sigma
